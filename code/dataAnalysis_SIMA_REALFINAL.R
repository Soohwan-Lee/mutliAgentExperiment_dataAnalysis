# =======================
# Multi-Agent Experiment: Analysis Script (updated)
# =======================
...
resp_flip <- resp1 %>%
  ...
  summarise(
    ...
    flip_support_to_opp = as.integer(!is.na(T0) && T0 > 0 &&
                                       any(opinion[t_ord > 0] < 0, na.rm = TRUE)),
...
for (d in agent_dims) {
  ...
  if (all(c(a1_col, a3_col) %in% names(post1))) {
    ...
    if (length(idx) > 0) {
      post1[[a_col]][idx] <- rowMeans(
        cbind(post1[[a1_col]][idx], post1[[a3_col]][idx]),
        na.rm = TRUE
      )
    }
  }
...
# -------------------------------------------------------
# 8) LMM: 행동(원값/델타), self-report, agent 인식
#     랜덤효과는 모두 (1 | participant_id)로 통일
# -------------------------------------------------------
...
#   SII_z + NFC_z + AIacc_z + (1 | participant_id)
# 
# m_opinion_raw <- lmer(form_op_raw, data = resp_behavior, REML = FALSE)
# m_conf_raw    <- lmer(form_cf_raw,  data = resp_behavior, REML = FALSE)
# 
# cat("\n=== Opinion (raw T0~T4) ===\n")
# print(summary(m_opinion_raw)); 
# 
# print(anova(m_opinion_raw, type = 3))
# 
# # 조건별 추정 평균 (논문용 테이블)
# emm_op_raw <- emmeans(m_opinion_raw, ~ pattern * task_type * timeF)
# print(emm_op_raw)
# 
# # Post-hoc - Pattern
...
# emmeans::eff_size(
#   emm_op_raw_pattern,
#   sigma = sigma(m_opinion_raw),
#   edf   = df.residual(m_opinion_raw)
# )
# 
# # Post-hoc - Task_type
# emm_op_raw_task_type <- emmeans(
#   m_opinion_raw,
#   specs = "task_type",
#   by   = c("pattern", "timeF")
# )
...
# emmeans::eff_size(
#   emm_op_raw_task_type,
#   sigma = sigma(m_opinion_raw),
#   edf   = df.residual(m_opinion_raw)
# )
# 
# # Post-hoc - timeF
# emm_op_raw_timeF <- emmeans(
#   m_opinion_raw,
#   specs = "timeF",
...
# 
# # 조건별 추정 평균 (논문용 테이블)
# emm_cf_raw <- emmeans(m_conf_raw, ~ pattern * task_type * timeF)
# print(emm_cf_raw)
# 
# # Post-hoc - Pattern
# emm_cf_raw_pattern <- emmeans(
#   m_conf_raw,
#   specs = "pattern",
#   by   = c("task_type", "timeF")
# )
...
# emmeans::eff_size(
#   emm_cf_raw_pattern,
#   sigma = sigma(m_conf_raw),
#   edf   = df.residual(m_conf_raw)
# )
# 
# # Post-hoc - task_type
# emm_cf_raw_task_type <- emmeans(
#   m_conf_raw,
#   specs = "task_type",
#   by   = c("pattern", "timeF")
# )
...
# emmeans::eff_size(
#   emm_cf_raw_task_type,
#   sigma = sigma(m_conf_raw),
#   edf   = df.residual(m_conf_raw)
# )
# 
# # Post-hoc - timeF
# emm_cf_raw_timeF <- emmeans(
#   m_conf_raw,
#   specs = "timeF",
#   by   = c("pattern", "task_type")
# )
...
m_opinion_signed <- lmer(form_op_signed, data = resp_delta, REML = FALSE)
m_conf_signed    <- lmer(form_cf_signed, data = resp_delta, REML = FALSE)
...
print(summary(m_opinion_signed)); 

print(anova(m_opinion_signed, type = 3))

emm_op_signed <- emmeans(m_opinion_signed, ~ pattern * task_type * timeF)
print(emm_op_signed)
...
print(emmeans::eff_size(
  ...
  sigma = sigma(m_opinion_signed),
  edf   = df.residual(m_opinion_signed)
))

# Post-hoc - Task_type
emm_op_signed_task_type <- emmeans(
  m_opinion_signed,
  specs = "task_type",
  by   = c("pattern","timeF"),
  data = resp_delta
...
print(emmeans::eff_size(
  emm_op_signed_task_type,
  sigma = sigma(m_opinion_signed),
  edf   = df.residual(m_opinion_signed)
))

# Post-hoc - timeF
emm_op_signed_timeF <- emmeans(
  m_opinion_signed,
  specs = "timeF",
  by   = c("pattern","task_type"),
  data = resp_delta
...
print(summary(m_conf_signed)); 

print(anova(m_conf_signed, type = 3))

emm_cf_signed <- emmeans(m_conf_signed, ~ pattern * task_type * timeF)
print(emm_cf_signed)

# Post-hoc - Pattern
emm_cf_signed_pattern <- emmeans(
  m_conf_signed,
  specs = "pattern",
  by   = c("task_type","timeF"),
  data = resp_delta
...
print(emmeans::eff_size(
  ...
  sigma = sigma(m_conf_signed),
  edf   = df.residual(m_conf_signed)
))

# Post-hoc - task_type
emm_cf_signed_task_type <- emmeans(
  m_conf_signed,
  specs = "task_type",
  by   = c("pattern","timeF"),
  data = resp_delta
...
print(emmeans::eff_size(
  emm_cf_signed_task_type,
  sigma = sigma(m_conf_signed),
  edf   = df.residual(m_conf_signed)
))

# Post-hoc - timeF
emm_cf_signed_timeF <- emmeans(
  m_conf_signed,
  specs = "timeF",
  by   = c("task_type","pattern"),
  data = resp_delta
)
...
m_opinion_abs <- lmer(form_op_abs, data = resp_delta, REML = FALSE)
m_conf_abs    <- lmer(form_cf_abs,  data = resp_delta, REML = FALSE)
...
cat("\n=== |ΔOpinion| (Tk−T0) ===\n")
print(summary(m_opinion_abs)); 

print(anova(m_opinion_abs, type = 3))


emm_op_abs <- emmeans(m_opinion_abs, ~ pattern * task_type * timeF)
print(emm_op_abs)
...
print(emmeans::eff_size(
  emm_op_abs_pattern,
  sigma = sigma(m_opinion_abs),
  edf   = df.residual(m_opinion_abs)
))

# Post-hoc - task_type
emm_op_abs_task_type <- emmeans(
  m_opinion_abs,
  specs = "task_type",
  by   = c("pattern","timeF"),
  data = resp_delta
...
print(emmeans::eff_size(
  emm_op_abs_task_type,
  sigma = sigma(m_opinion_abs),
  edf   = df.residual(m_opinion_abs)
))

# Post-hoc - timeF
emm_op_abs_timeF <- emmeans(
  m_opinion_abs,
  specs = "timeF",
  by   = c("task_type","pattern"),
  data = resp_delta
...
cat("\n=== |ΔConfidence| (Tk−T0) ===\n")
print(summary(m_conf_abs)); 

print(anova(m_conf_abs, type = 3))

emm_cf_abs <- emmeans(m_conf_abs, ~ pattern * task_type * timeF)
print(emm_cf_abs)

# Post-hoc - Pattern
emm_cf_abs_pattern <- emmeans(
  m_conf_abs,
  specs = "pattern",
  by   = c("task_type","timeF"),
  data = resp_delta
...
print(emmeans::eff_size(
  emm_cf_abs_pattern,
  sigma = sigma(m_conf_abs),
  edf   = df.residual(m_conf_abs)
))

# Post-hoc - task_type
emm_cf_abs_task_type <- emmeans(
  m_conf_abs,
  specs = "task_type",
  by   = c("pattern","timeF"),
  data = resp_delta
...
print(emmeans::eff_size(
  emm_cf_abs_task_type,
  sigma = sigma(m_conf_abs),
  edf   = df.residual(m_conf_abs)
))

# Post-hoc - timeF
emm_cf_abs_timeF <- emmeans(
  m_conf_abs,
  specs = "timeF",
  by   = c("task_type","pattern"),
  data = resp_delta
...
# ---------- 8-4) Self-report: compliance / conversion LMM ----------
m_comp <- lmer(
  compliance_mean ~ pattern * task_type + SII_z + NFC_z + AIacc_z +
    (1 | participant_id),
  data = post1, REML = TRUE
)
m_conv <- lmer(
  conversion_mean ~ pattern * task_type + SII_z + NFC_z + AIacc_z +
    (1 | participant_id),
  data = post1, REML = TRUE
)
...
# Post-hoc: Pattern
emm_comp_pattern <- emmeans(
  m_comp,
  specs = "pattern",
  by   = "task_type",
  data = post1
)
print(pairs(emm_comp_pattern, adjust="bonferroni"))
print(emmeans::eff_size(
  emm_comp_pattern,
  sigma = sigma(m_comp),
  edf   = df.residual(m_comp)
))

# Post-hoc: Task-type
emm_comp_task <- emmeans(
  m_comp,
  specs = "task_type",
  by   = "pattern",
  data = post1
)
print(pairs(emm_comp_task, adjust="bonferroni"))
print(emmeans::eff_size(
  emm_comp_task,
  sigma = sigma(m_comp),
  edf   = df.residual(m_comp)
...
print(summary(m_conv)); 
print(anova(m_conv, type = 3))

emm_conv_table <- emmeans(m_conv, ~ pattern * task_type)
print(emm_conv_table)

# Post-hoc: Pattern
emm_conv_pattern <- emmeans(
  m_conv,
  specs = "pattern",
  by   = "task_type",
  data = post1
)
print(pairs(emm_conv_pattern, adjust="bonferroni"))
print(emmeans::eff_size(
  emm_conv_pattern,
  sigma = sigma(m_conv),
  edf   = df.residual(m_conv)
))

# Post-hoc: Task-Type
emm_conv_task <- emmeans(
  m_conv,
  specs = "task_type",
  by   = "pattern",
  data = post1
)
print(pairs(emm_conv_task, adjust="bonferroni"))
print(emmeans::eff_size(
  emm_conv_task,
  sigma = sigma(m_conv),
  edf   = df.residual(m_conv)
